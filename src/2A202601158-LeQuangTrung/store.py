...
class EmbeddingStore:
    ...
    def _make_record(self, doc: Document) -> dict[str, Any]:
        ...
        # A top-level Document is also a valid chunk, so establish the field
        # used by delete_document even when ingest.py was not involved.
    ...
    def add_documents(self, docs: list[Document]) -> None:
        ...
        if self._use_chroma and self._collection is not None:
            try:
                ...
                # ChromaDB is optional.  Falling back retains the same public
                # behaviour if it rejects an environment or metadata value.
                self._use_chroma = False
                self._collection = None
    ...
    def delete_document(self, doc_id: str) -> bool:
        ...
        if self._use_chroma and self._collection is not None:
            try:
                ...
                # The in-memory cache remains authoritative for the current
                # instance, so a failed optional backend does not break delete.
                self._use_chroma = False
                self._collection = None
